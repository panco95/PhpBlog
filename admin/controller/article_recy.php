...
if(empty($_SESSION['username']) && empty($_SESSION['uid'])){
    echo "<script>alert('恢复文章失败！原因：没有登录！');setTimeout(window.location.href='../admin.php?list=article_manger',5);</script>";
}else {
...
//PDO预处理中insert delete update等语句，用rowCount()检测是否执行成功
        ...
        echo "<script>alert('删除文章失败！');setTimeout(window.location.href='../admin.php?list=article_recybin',5);</script>";
    }
}